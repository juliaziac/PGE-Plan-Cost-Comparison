...
def cost_comparison_quarterly_one_year(total: pd.DataFrame, row_index: int) -> pd.DataFrame:
    ...
    Args:
        total (pd.DataFrame): A dataframe of the gas and electric data combined, with the rolling 12-month
            cost average computed, generated from the combine_and_process function.
...
def total_analysis(combined_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(start_month_index, len(combined_df.index)-12):
        df = df._append(cost_comparison_yearly(cost_comparison_quarterly_one_year(combined_df, i)), ignore_index=True)
    return df